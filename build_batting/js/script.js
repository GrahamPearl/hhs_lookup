...
document.addEventListener("DOMContentLoaded", function () {
  // Absence checker (NEW)
  const absenceTeacherSelect = document.getElementById("absenceTeacherSelect");
  const absenceDaySelect = document.getElementById("absenceDaySelect");
  const checkAvailabilityBtn = document.getElementById("checkAvailabilityBtn");
  const availabilityResults = document.getElementById("availabilityResults");

  const bulkImportBtn = document.getElementById("bulkImportBtn");
  const bulkImportFolderInput = document.getElementById(
    "bulkImportFolderInput",
  );
  ...
  function removeTeacherFromIndexByKey(storageKey) {
    const list = loadTeacherIndex();
    const newList = list.filter((item) => item.key !== storageKey);
    saveTeacherIndex(newList);
  }

  function refreshAllTeacherDropdowns() {
    // Refresh main teacher dropdown (saved timetables)
    refreshTeacherSelect(getTeacherStorageKey(currentTeacherName));

    // Refresh absence dropdown if it exists in your build
    if (typeof refreshAbsenceTeacherSelect === "function") {
      refreshAbsenceTeacherSelect();
    }
  }

  function getPayloadByStorageKey(storageKey) {
    const raw = localStorage.getItem(storageKey);
    if (!raw) return null;
    try {
      return JSON.parse(raw);
    } catch (e) {
      console.error("Bad payload JSON for", storageKey, e);
      return null;
    }
  }
  ...
  function refreshAbsenceTeacherSelect(selectedKey = "") {
    const list = loadTeacherIndex();

    absenceTeacherSelect.innerHTML = "";
    const defaultOpt = document.createElement("option");
    defaultOpt.value = "";
    defaultOpt.textContent = list.length
      ? "-- Select a teacher --"
      : "-- No saved timetables --";
    absenceTeacherSelect.appendChild(defaultOpt);

    list.forEach((t) => {
      const opt = document.createElement("option");
      opt.value = t.key;
      opt.textContent = t.name;
      absenceTeacherSelect.appendChild(opt);
    });

    if (selectedKey) absenceTeacherSelect.value = selectedKey;

    // reset day selection
    absenceDaySelect.innerHTML = `<option value="">-- Select teacher first --</option>`;
    absenceDaySelect.disabled = true;
    checkAvailabilityBtn.disabled = true;
  }

  function populateAbsenceDaySelect(payload) {
    const rows = payload?.config?.rows || 0;
    const dayNames = payload?.config?.dayNames || [];

    absenceDaySelect.innerHTML = "";

    const defaultOpt = document.createElement("option");
    defaultOpt.value = "";
    defaultOpt.textContent = "-- Select a day --";
    absenceDaySelect.appendChild(defaultOpt);

    for (let r = 0; r < rows; r++) {
      const opt = document.createElement("option");
      opt.value = String(r);
      opt.textContent = dayNames[r] || `Day ${r + 1}`;
      absenceDaySelect.appendChild(opt);
    }

    absenceDaySelect.disabled = false;
  ...
  function deleteSelectedTeacherTimetable() {
    // Priority: selected in combobox, otherwise use teacher name input
    const selectedKey =
      teacherSelect && teacherSelect.value ? teacherSelect.value : "";
    const teacherName = teacherNameInput ? teacherNameInput.value.trim() : "";

    let storageKey = selectedKey;
    if (!storageKey && teacherName) {
      storageKey = getTeacherStorageKey(teacherName);
    }

    if (!storageKey) {
      alert(
        "Please select a teacher from the Saved timetables list (or enter a teacher name) to delete.",
      );
      return;
    }
    ...
    // Confirm
    const displayName =
      teacherSelect?.selectedOptions?.[0]?.textContent ||
      teacherName ||
      "this teacher";
    if (
      !confirm(
        `Delete the stored timetable for: ${displayName}? This cannot be undone.`,
      )
    )
      return;
    ...
    // Remove from index
    removeTeacherFromIndexByKey(storageKey);

    // If we just deleted the currently loaded teacher, clear current view
    const currentKey = getTeacherStorageKey(currentTeacherName);
    ...
    alert("Teacher timetable deleted.");
  }

  function deleteAllTeacherTimetables() {
    const list = loadTeacherIndex();

    if (list.length === 0) {
      alert("There are no saved teacher timetables to delete.");
      return;
    }

    if (
      !confirm(
        `Delete ALL stored teacher timetables (${list.length})? This cannot be undone.`,
      )
    )
      return;

    // Remove each teacher timetable by key
    list.forEach((item) => {
    ...
    // Clear current loaded timetable view
    currentTeacherName = "";
    if (teacherNameInput) teacherNameInput.value = "";
    timetableConfig = { rows: 0, cols: 0, dayNames: [] };
    timetableData = {};
    ...
    // Refresh dropdowns (they should now be empty)
    refreshAllTeacherDropdowns();

    alert("All teacher timetables have been deleted.");
  }
  ...
  async function bulkImportFromFileList(fileList) {
    ...
    bulkImportStatus.textContent = `Importing ${files.length} JSON file(s)...`;
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        // 1) payload.teacherName
        // 2) filename without extension
        let teacherName = (payload.teacherName || "").trim();
        if (!teacherName) {
          teacherName = file.name.replace(/\.json$/i, "").trim();
        }
        ...
        // Normalise to your expected structure
        const config = payload.config || {};
        const entries = Array.isArray(payload.entries) ? payload.entries : [];
        ...
        // Add to teacher list + refresh combobox
        ensureTeacherInIndex(teacherName);

        imported++;
      } catch (err) {
        console.error("Bulk import error:", file.name, err);
        failed++;
      }

      // Live progress
      bulkImportStatus.textContent = `Importing... (${i + 1}/${files.length}) Imported: ${imported}, Skipped: ${skipped}, Failed: ${failed}`;
    }

    // Refresh dropdowns after bulk import
    refreshTeacherSelect();
    if (typeof refreshAbsenceTeacherSelect === "function") {
      refreshAbsenceTeacherSelect();
    }

    bulkImportStatus.textContent = `Done. Imported: ${imported}, Skipped: ${skipped}, Failed: ${failed}.`;
  }
  ...
  function computeAvailability(absentTeacherKey, dayIndex) {
    const teacherList = loadTeacherIndex();
    const absentPayload = getPayloadByStorageKey(absentTeacherKey);

    if (!absentPayload) return null;

    const cols = absentPayload?.config?.cols || 0;
    const availability = Array.from({ length: cols }, () => []);

    teacherList.forEach((teacher) => {
      // Skip the absent teacher (optional)
      if (teacher.key === absentTeacherKey) return;

      const payload = getPayloadByStorageKey(teacher.key);
      if (!payload) return;

      const tRows = payload?.config?.rows || 0;
      const tCols = payload?.config?.cols || 0;

      // If teacher timetable doesn't include this day, skip them
      if (dayIndex >= tRows) return;

      const entryMap = buildEntryMap(payload.entries);

      for (let c = 0; c < cols; c++) {
        if (c >= tCols) continue; // teacher doesn't have that many periods

        const entry = entryMap[`${dayIndex}-${c}`];

        // Available ONLY if explicitly marked as free
        if (entry && entry.type === "free") {
          availability[c].push({ name: teacher.name, key: teacher.key });
        }
      }
    });

    return {
      absent: absentPayload.teacherName || "(Unknown)",
      config: absentPayload.config,
      dayIndex,
      availability,
    };
  }
  ...
  function renderAvailability(result) {
    if (!result) {
      availabilityResults.innerHTML = `<div class="alert alert-warning mb-0">No data available.</div>`;
      return;
    }

    const cols = result.config?.cols || 0;
    const dayNames = result.config?.dayNames || [];
    const dayLabel = dayNames[result.dayIndex] || `Day ${result.dayIndex + 1}`;

    let html = `
      <div class="mb-2">
        <div class="fw-bold">Absent: ${result.absent}</div>
        <div class="text-muted small">Checking availability for: ${dayLabel}</div>
      </div>
        ...
        <table class="table table-sm table-bordered align-middle">
          <thead class="table-light">
            <tr>
              ...
              <th>Available teachers (Free Time)</th>
            </tr>
          </thead>
          <tbody>
    `;
    ...
    for (let c = 0; c < cols; c++) {
      const available = result.availability[c];
      html += `<tr>
            <th>P${c + 1}</th>
            <td>`;
      ...
      if (!available.length) {
        ...
        available.forEach((t) => {
          // clickable badge to load that teacher's timetable
          html += `
                  <button type="button"
                          class="btn btn-sm btn-outline-success me-1 mb-1 availability-teacher"
                          data-teacher-key="${t.key}">
    ...
    // Add click handlers: clicking a teacher loads their timetable
    availabilityResults
      .querySelectorAll(".availability-teacher")
      .forEach((btn) => {
        btn.addEventListener("click", () => {
          const key = btn.getAttribute("data-teacher-key");
          if (key) loadTimetableFromStorageKey(key);
        });
      });
  }
  ...
  deleteTeacherBtn.addEventListener("click", deleteSelectedTeacherTimetable);
  deleteAllBtn.addEventListener("click", deleteAllTeacherTimetables);

  absenceTeacherSelect.addEventListener("change", function () {
    const key = absenceTeacherSelect.value;
    ...
    const payload = getPayloadByStorageKey(key);
    if (!payload) {
      availabilityResults.innerHTML = `<div class="alert alert-danger mb-0">Could not load this teacher's timetable.</div>`;
      return;
    }

    populateAbsenceDaySelect(payload);
  });

  checkAvailabilityBtn.addEventListener("click", function () {
    const teacherKey = absenceTeacherSelect.value;
    const dayIndex = parseInt(absenceDaySelect.value, 10);

    if (!teacherKey || isNaN(dayIndex)) {
      availabilityResults.innerHTML = `<div class="alert alert-warning mb-0">Please select an absent teacher and a day.</div>`;
      return;
    }

    const result = computeAvailability(teacherKey, dayIndex);
    renderAvailability(result);
  });

  bulkImportBtn.addEventListener("click", () => {
    bulkImportFolderInput.click();
  });

  bulkImportFolderInput.addEventListener("change", async () => {
    bulkImportStatus.textContent = "";
    await bulkImportFromFileList(bulkImportFolderInput.files);
    bulkImportFolderInput.value = ""; // reset so same folder can be re-selected
  });
