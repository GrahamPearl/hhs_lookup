...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    body2.addEventListener("shown.bs.collapse",  () => { btn2.setAttribute("aria-expanded","true");  icon2.textContent="▼"; });
    body2.addEventListener("hidden.bs.collapse", () => { btn2.setAttribute("aria-expanded","false"); icon2.textContent="▶"; });
  });

  const collapse = bootstrap.Collapse.getOrCreateInstance(body, { toggle: false });
  if (expanded) collapse.show(); else collapse.hide();
  btn.setAttribute("aria-expanded", expanded ? "true" : "false");
  icon.textContent = expanded ? "▼" : "▶";
  body.addEventListener("shown.bs.collapse",  () => { localStorage.setItem(DASHBOARD_BODY_KEY,"true");  btn.setAttribute("aria-expanded","true");  icon.textContent="▼"; });
  body.addEventListener("hidden.bs.collapse", () => { localStorage.setItem(DASHBOARD_BODY_KEY,"false"); btn.setAttribute("aria-expanded","false"); icon.textContent="▶"; });
});
...
function saveFairnessSettings(s) { localStorage.setItem(FAIRNESS_SETTINGS_KEY, JSON.stringify(s)); }
function getFairnessSettings()   { return loadFairnessSettings(); }

// ── Smart defaults ─────────────────────────────────────────────
function saveLastStrategy(s) { localStorage.setItem(LAST_STRATEGY_KEY, s); }
function getLastStrategy()   { return localStorage.getItem(LAST_STRATEGY_KEY) || "fair"; }
function saveLastReason(r)   { localStorage.setItem(LAST_REASON_KEY, r); }
function getLastReason()     { return localStorage.getItem(LAST_REASON_KEY) || ""; }

// ── Absence Reason Management ──────────────────────────────────
function loadAbsenceReasons() {
  const stored = localStorage.getItem(ABSENCE_REASONS_KEY);
  if (stored) { try { return JSON.parse(stored); } catch { return DEFAULT_ABSENCE_REASONS; } }
  return DEFAULT_ABSENCE_REASONS;
}
function saveAbsenceReasons(r) { localStorage.setItem(ABSENCE_REASONS_KEY, JSON.stringify(r)); }
function getAbsenceReasons()   { return loadAbsenceReasons(); }

function addAbsenceReason(reason) {
  if (!reason.trim()) return false;
  const reasons = loadAbsenceReasons();
  const trimmed = reason.trim();
  if (!reasons.includes(trimmed)) { reasons.push(trimmed); saveAbsenceReasons(reasons); return true; }
  return false;
}
function removeAbsenceReason(reason) {
  const reasons  = loadAbsenceReasons();
  const filtered = reasons.filter(r => r !== reason);
  if (filtered.length < reasons.length) { saveAbsenceReasons(filtered); return true; }
  return false;
}
...
function loadMetrics() {
  if (_metricsCache === null) _metricsCache = JSON.parse(localStorage.getItem(METRICS_KEY) || "{}");
  return _metricsCache;
}
function saveMetrics(m) { _metricsCache = m; localStorage.setItem(METRICS_KEY, JSON.stringify(m)); }

function loadCoverHistory() {
  if (_historyCache === null) _historyCache = JSON.parse(localStorage.getItem(HISTORY_KEY) || "[]");
  return _historyCache;
}
function saveCoverHistory(h) { _historyCache = h; localStorage.setItem(HISTORY_KEY, JSON.stringify(h)); }

function getCachedTenWeekStart() {
  if (_tenWeekStartCache === undefined) _tenWeekStartCache = localStorage.getItem(TEN_WEEK_START);
  return _tenWeekStartCache;
}
function setCachedTenWeekStart(val) {
  _tenWeekStartCache = val; _startDateObj = null;
...
window.addEventListener("storage", e => {
  if      (e.key === METRICS_KEY) _metricsCache = null;
    ...
    delete teacherCache[name]; delete _entryIndex[name]; _teacherNamesCache = null;
  }
});

// ── 10-week period helpers ─────────────────────────────────────
function initializeTenWeekPeriod() {
  if (!getCachedTenWeekStart()) setCachedTenWeekStart(new Date().toISOString().split("T")[0]);
}
function getWeekNumber(dateStr) {
  if (!_startDateObj) { initializeTenWeekPeriod(); _startDateObj = new Date(getCachedTenWeekStart()); }
...
function countCoversForTeacherOnDay(name, day) {
  return loadCoverHistory().filter(h =>
    h.coverTeacher === name && new Date(h.date).toISOString().split("T")[0] === coverDate
  ).length;
}
function countCoversForTeacherThisWeek(name) {
  const week = getWeekNumber(coverDate);
  return loadCoverHistory().filter(h => h.coverTeacher === name && h.week === week).length;
}
...
function buildHistoryStats(history, currentWeek) {
  ...
  for (let i = 0, len = history.length; i < len; i++) {
    const h = history[i], t = h.coverTeacher;
  ...
  for (const t in stats)
    stats[t].coversPerWeek = currentWeek > 0 ? (stats[t].relevantTotal / currentWeek).toFixed(2) : "0.00";
  return stats;
}
...
function getCoversPerWeekAverage(coverTeacher) {
  const history = loadCoverHistory(), week = getWeekNumber(coverDate);
  let count = 0;
  for (let i = 0, len = history.length; i < len; i++)
    if (history[i].coverTeacher === coverTeacher && history[i].week <= week) count++;
...
function loadTeacher(name) {
  if (!teacherCache[name]) {
    ...
    if (raw) { const data = JSON.parse(raw); teacherCache[name] = data; _buildEntryIndex(name, data); }
  }
...
function ensureTeacherMetrics(name) {
  ...
  if (!metrics[name]) {
    metrics[name] = { freePeriods: calculateFreePeriods(name), coversDone: 0, coversThisWeek: 0, totalCovers: 0, lastCoverDate: null };
    saveMetrics(metrics);
  }
}
...
function autoPruneOldEntries() {
  const history = loadCoverHistory(), tenWeekStart = getCachedTenWeekStart();
...
function getAllAvailableTeachers(day, absentList) {
  ...
  for (const key in coverAssignments) {
    ...
    if (idx === -1) continue;
    const dp = key.slice(idx + 1), dashIdx = dp.indexOf("-");
    const d = parseInt(dp.slice(0, dashIdx)), p = parseInt(dp.slice(dashIdx + 1));
    if (d === day) { if (!assignedByPeriod[p]) assignedByPeriod[p] = new Set(); assignedByPeriod[p].add(coverAssignments[key]); }
  }
  const absentSet = new Set(absentList), allNames = getTeacherNames(), result = {};
  ...
  for (let p = 0; p < 6; p++) {
    for (let j = 0; j < result[p].length; j++) {
      ...
      seen.add(tName);
      if (!metrics[tName]) {
        metrics[tName] = { freePeriods: calculateFreePeriods(tName), coversDone: 0, coversThisWeek: 0, totalCovers: 0, lastCoverDate: null };
        metricsChanged = true;
      }
    }
  }
  ...
  const history = loadCoverHistory(), currentWeek = getWeekNumber(coverDate);
  const histStats = buildHistoryStats(history, currentWeek);

  for (let p = 0; p < 6; p++) {
    const filtered = result[p].filter(t => {
      if (settings.excludeDnd && isTeacherDnd(t.name, day, p)) return false;
      if (countCoversForTeacherOnDay(t.name, day) >= settings.maxCoversPerDay) return false;
      if (countCoversForTeacherThisWeek(t.name) >= settings.maxCoversPerWeek) return false;
      return true;
    });

    let regularTeachers = [], lastResortTeachers = [];
    ...
    regularTeachers.sort(sortFn); lastResortTeachers.sort(sortFn);
...
function refreshTeachers() {
  ...
  if (!sel) return;
  const absentSet = new Set([...absentTeachers, ...Object.keys(partialAbsentTeachers)]);
...
function openAddAbsenceModal(name) {
  ...
  document.querySelectorAll(".add-period-checkbox").forEach(cb => cb.checked = false);
...
document.getElementById("absenceTypePartial")?.addEventListener("change", () => {
...
// Confirm absence
document.getElementById("confirmAddAbsenceBtn")?.addEventListener("click", () => {
  ...
  if (isPartial) {
    ...
    document.querySelectorAll(".add-period-checkbox:checked").forEach(cb =>
...
function renderAbsentTeachersTable() {
  ...
  tableBody.innerHTML = absentTeachers
    .map((name, idx) => {
      const reason = absentTeacherReasons[name] ? `<br><small class="text-muted fst-italic">${absentTeacherReasons[name]}</small>` : "";
      return `<tr>
        <td>${name}${reason}</td>
...
function renderPartialAbsentList() {
  ...
  const partialList = Object.entries(partialAbsentTeachers)
    .map(([name, periods]) => {
      ...
      const reason       = absentTeacherReasons[name] ? `<small class="text-muted fst-italic d-block">${absentTeacherReasons[name]}</small>` : "";
      return `
  ...
  list.innerHTML = partialList || '<div class="text-muted small">No partial absences</div>';
}
...
function scheduleRenderGrid() {
  ...
  _renderGridRAF = requestAnimationFrame(() => { _renderGridRAF = null; renderGrid(); });
}
...
function updateDashboardSummary() {
  ...
  if (history.length === 0) {
    ...
    document.getElementById("dashboardTrend").textContent        = "No data yet. Complete assignments to see fairness metrics.";
    return;
  }
  const totals = {};
  for (const h of history) totals[h.coverTeacher] = (totals[h.coverTeacher] || 0) + 1;
  ...
  document.getElementById("teacherCountDisplay").textContent   = Object.keys(totals).length;
  document.getElementById("imbalanceRatioDisplay").textContent = ratio;

  let trend = "";
  if      (score >= 85) trend = "✓ Excellent fairness balance. Continue current strategy.";
  else if (score >= 70) trend = "⚠️ Good fairness. Monitor for imbalance in next assignments.";
  else if (score >= 50) trend = "⚠️ Fair balance. Consider using Day-Balancing strategy.";
  else                  trend = "⚠️ Imbalanced. Strongly recommend Fair or Day-Balancing strategy.";
  document.getElementById("dashboardTrend").textContent = trend;
}

function hasAnyAbsences() {
  return absentTeachers.length > 0 || Object.keys(partialAbsentTeachers).length > 0;
}
...
function getCoverNeededLessons(day) {
  ...
  absentTeachers.forEach(teacher => {
    ...
    data.entries.filter(e => e.row == day && e.type === "lesson").sort((a,b) => a.col - b.col).forEach(e => {
      if (e.col === 6) return;
      items.push({ coveredTeacher: teacher, periodCol: e.col, entry: e, absenceType: "full", key: `${teacher}:${day}-${e.col}` });
    });
  });
  Object.entries(partialAbsentTeachers).forEach(([teacher, absentPeriods]) => {
    const data = loadTeacher(teacher);
    if (!data) return;
    data.entries.filter(e => e.row == day && e.type === "lesson" && absentPeriods.includes(e.col)).sort((a,b) => a.col - b.col).forEach(e => {
      if (e.col === 6) return;
      items.push({ coveredTeacher: teacher, periodCol: e.col, entry: e, absenceType: "partial", key: `${teacher}:${day}-${e.col}` });
    });
  });
  items.sort((a,b) => a.periodCol - b.periodCol || a.coveredTeacher.localeCompare(b.coveredTeacher));
  return items;
}

// ── Main cover grid ────────────────────────────────────────────
function renderGrid() {
  if (_renderGridRAF !== null) { cancelAnimationFrame(_renderGridRAF); _renderGridRAF = null; }
  ...
  function buildTeacherRows(teacher, lessons, badgeHtml) {
    ...
    lessons.forEach(e => {
      ...
      rowsHtml.push(`<tr><td>${e.col+1}</td><td>${e.subject||e.type}</td><td>${e.className||""}</td><td>${e.venue||""}</td><td>${assignHtml}</td></tr>`);
    });
    rowsHtml.push(`<tr><td colspan="6" style="height:10px;background-color:#f8f9fa;"></td></tr>`);
  }

  // Full-day absent teachers
  absentTeachers.forEach(teacher => {
    const data = loadTeacher(teacher);
    if (!data) return;
    const lessons = data.entries.filter(e => e.row == day && e.type === "lesson").sort((a,b) => a.col - b.col);
    buildTeacherRows(teacher, lessons, `<span class='badge bg-danger'>Full Day</span>`);
  });

  // Partial-day absent teachers
  Object.entries(partialAbsentTeachers).forEach(([teacher, absentPeriods]) => {
    const data = loadTeacher(teacher);
    if (!data) return;
    const lessons  = data.entries.filter(e => e.row == day && e.type === "lesson" && absentPeriods.includes(e.col)).sort((a,b) => a.col - b.col);
    const periodLabels = absentPeriods.map(p => `P${p+1}`).join(",");
    buildTeacherRows(teacher, lessons, `<span class='badge bg-warning text-dark'>Partial: ${periodLabels}</span>`);
  });
  ...
  for (let period = 0; period < 6; period++) {
    ...
    let tdContent = avail.length === 0
      ...
      : avail.map(teacher => {
          ...
          // Colour priority: lastFree (warning) > normal free/meeting
          const badgeColor = lastFree
            ? "bg-warning text-dark"
            : teacher.type === "free" ? "bg-primary" : "bg-secondary";

          // Border priority: overloaded (danger) > coveredPrior (info)
          const borderClass = overloaded
            ? " border border-danger"
            : coveredPrior ? " border border-info border-2" : "";

          // Compose tooltip
          const titleParts = [];
          if (lastFree)     titleParts.push("⚠️ Last available period today");
          if (coveredPrior) titleParts.push("↩ Covered on " + priorDateStr);
          if (overloaded)   titleParts.push("🔴 High cover load (>5 total)");
          const titleAttr = titleParts.length ? ` title="${titleParts.join(" · ")}"` : "";

          // Inline indicators appended to name
          const nameIndicators = [
            lastFree     ? "⚠️" : "",
            coveredPrior ? "↩"  : "",
          ].filter(Boolean).join(" ");

          return `<span class="badge me-1 avail-badge ${badgeColor}${borderClass}" draggable="true" data-teacher-name="${teacher.name}"${titleAttr}>
  ${teacher.name}${nameIndicators ? " " + nameIndicators : ""}
...
function checkFairnessWarnings() {
  const history = loadCoverHistory(), week = getWeekNumber(coverDate), settings = getFairnessSettings();
  const coversPerTeacher = {};
  for (let i = 0, len = history.length; i < len; i++)
    if (history[i].week === week) coversPerTeacher[history[i].coverTeacher] = (coversPerTeacher[history[i].coverTeacher]||0)+1;

  const warnings = [];
  for (const teacher in coversPerTeacher)
    if (coversPerTeacher[teacher] > settings.maxCoversPerWeek)
      warnings.push(`⚠️ ${teacher} has ${coversPerTeacher[teacher]} covers this week (exceeds fair limit of ${settings.maxCoversPerWeek})`);

  const constraintInfo = [];
  if (settings.excludeDnd)    constraintInfo.push("DND teachers excluded");
  if (settings.useLastResort) constraintInfo.push("Last Resort used only as backup");
  if (constraintInfo.length > 0) warnings.push(`ℹ️ Active fairness constraints: ${constraintInfo.join(", ")}`);

  const warningDiv = document.getElementById("fairnessWarning");
  if (warnings.length > 0) { warningDiv.innerHTML = warnings.join("<br>"); warningDiv.classList.remove("d-none"); }
...
function undo(key) {
  ...
  saveCoverHistory(history.filter(h => !(h.coveredTeacher===teacher && h.day===dayNum && h.period===periodNum)));
  delete coverAssignments[key];
  renderGrid();
}
function markNoCover(key)  { noCoverNeeded[key] = true;  renderGrid(); }
function undoNoCover(key)  { delete noCoverNeeded[key];  renderGrid(); }
...
function autoAssignCoverTeachers() {
  ...
  absentTeachers.forEach(teacher => {
    ...
    data.entries.filter(e => e.row == day && e.type === "lesson").sort((a,b)=>a.col-b.col).forEach(e => {
      ...
      let availableTeachers = getAvailableTeachers(e.col, day, absentTeachers).filter(t => !assignedTeachers.has(t.name));
      if (availableTeachers.length > 0) {
        const best = availableTeachers[0];
        coverAssignments[key] = best.name; assignedTeachers.add(best.name);
        addCoverHistoryEntry(teacher, best.name, e.col+1, day+1, e.subject||e.type, e.className, e.venue);
        if (!metrics[best.name]) metrics[best.name] = { freePeriods:calculateFreePeriods(best.name), coversDone:0, coversThisWeek:0, totalCovers:0, lastCoverDate:null };
...
function initializeDatePicker() { document.getElementById("coverDate").value = coverDate; updateWeekDisplay(); }

// ── Period modal ───────────────────────────────────────────────
function initializePeriodModal() {
  const startDate = getCachedTenWeekStart();
  document.getElementById("tenWeekStartDate").value = startDate || new Date().toISOString().split("T")[0];
  updatePeriodStatus();
}
function updatePeriodStatus() {
  const startDate = getCachedTenWeekStart();
  if (!startDate) { document.getElementById("periodStatus").innerHTML = "Not set. Will initialize on first use."; return; }
  ...
  document.getElementById("periodStatus").innerHTML = `Started: ${start.toDateString()}<br>Ends: ${end.toDateString()}<br>Week: ${Math.min(weeksElapsed,10)} of 10`;
}
...
function populateLast5DaysModal() {
  ...
  const history = loadCoverHistory();
  if (history.length === 0) {
    body.innerHTML = "<p class='text-muted text-center py-3'>No cover history recorded yet.</p>";
    return;
  }
  const byDate = {};
  ...
  const dates = Object.keys(byDate).sort((a,b) => new Date(b) - new Date(a)).slice(0, 5);
  let html = "";
  dates.forEach(date => {
    const entries = byDate[date].sort((a,b) => (a.period||0) - (b.period||0));
    const weekNum = entries[0]?.week || "?";
    const fmt = new Date(date + "T00:00:00").toLocaleDateString(undefined, { weekday:"long", year:"numeric", month:"short", day:"numeric" });
    html += `
      <div class="card mb-3">
...
function displayCoverHistory() {
  const history = loadCoverHistory(), tbody = document.getElementById("historyTableBody");
  if (history.length === 0) {
...
function getCoverPlanRows(day) {
  const rows = [], coverNeeded = getCoverNeededLessons(day);
...
function openCoverPrintPreview(action = null) {
  if (!hasAnyAbsences()) { alert("No absent teachers selected. Please add absent teachers first."); return; }
  const day      = parseInt(document.getElementById("absenceDaySelect").value);
  ...
  const setupActions = () => {
    try {
      ...
      const rows = getCoverPlanRows(day);
      const makeText = () => rows.map(r => `${r.teacher} | P${r.period} | ${r.subject} | ${r.className} | ${r.venue} | ${r.assigned}`).join("\n");
      doc.getElementById("printPageBtn").onclick = () => win.print();
      const ensureLibs = () => {
        const promises = [];
        if (!win.html2canvas) promises.push(new Promise((res,rej)=>{ const s=doc.createElement("script"); s.src=HTML2CANVAS_URL; s.onload=res; s.onerror=rej; doc.head.appendChild(s); }));
        if (!win.jspdf)       promises.push(new Promise((res,rej)=>{ const s=doc.createElement("script"); s.src=JSPDF_URL; s.onload=res; s.onerror=rej; doc.head.appendChild(s); }));
      ...
      doc.getElementById("downloadPdfBtn").onclick = () => {
        ensureLibs().then(() => {
          ...
          win.html2canvas(content, { scale: 2 }).then(canvas => {
            ...
            if (imgH <= pdfH) { pdf.addImage(imgData,"PNG",0,0,imgW,imgH); }
            else {
              let remaining = canvas.height, pos = 0;
              while (remaining > 0) {
                const pageH = Math.min(remaining, Math.floor(pdfH/ratio));
                const c = document.createElement("canvas"); c.width=canvas.width; c.height=pageH;
      ...
      doc.getElementById("downloadPngBtn").onclick = () => {
        ensureLibs().then(() => {
          win.html2canvas(doc.querySelector(".container"),{scale:2}).then(canvas => {
            const link = doc.createElement("a"); link.download=`cover_plan_day_${day+1}.png`; link.href=canvas.toDataURL("image/png"); link.click();
    ...
    } catch(err) { console.error("setupActions failed",err); }
  };
...
document.getElementById("coverDate").addEventListener("change", e => {
  coverDate = e.target.value; updateWeekDisplay(); scheduleRenderGrid();
});
...
function autoAssignRoundRobin() {
  const day = parseInt(document.getElementById("absenceDaySelect").value);
  const allAvail = getAllAvailableTeachers(day, absentTeachers);
  const assignments = {}, noCoverAssignments = {};
  let assignmentsMade = 0, conflicts = 0;
  const allTeachers = [];
  ...
  const processTeacher = (teacher, lessons) => {
    lessons.forEach(e => {
      ...
      while (attempts < allTeachers.length) {
        ...
        teacherIndex++; attempts++;
        const candidateAvail = allAvail[e.col].find(t => t.name === candidate.name && candidate.name !== teacher);
        if (candidateAvail) { assignments[key] = candidate.name; assignmentsMade++; assigned = true; break; }
      }
  ...
  absentTeachers.forEach(teacher => {
    const data = loadTeacher(teacher); if (!data) return;
    processTeacher(teacher, data.entries.filter(e => e.row==day && e.type==="lesson").sort((a,b)=>a.col-b.col));
  });
  Object.entries(partialAbsentTeachers).forEach(([teacher, absentPeriods]) => {
    const data = loadTeacher(teacher); if (!data) return;
    processTeacher(teacher, data.entries.filter(e => e.row==day && e.type==="lesson" && absentPeriods.includes(e.col)).sort((a,b)=>a.col-b.col));
  });
...
function previewAutoAssign() {
  if (!hasAnyAbsences()) { alert("No absent teachers selected."); return; }
  ...
  preAutoAssignState    = { coverAssignments: {...coverAssignments}, noCoverNeeded: {...noCoverNeeded} };

  const conflicts = detectBackToBackConflicts(previewAssignments);
  const day = parseInt(document.getElementById("absenceDaySelect").value);
  const strategyLabel = strategy==="roundRobin" ? "Round-Robin (Sequential)" : strategy==="dayBalancing" ? "Day-Balancing (Spread Across Day)" : "Fair (Minimize Unfairness)";
  ...
  const appendRows = (teacher, entries, badgeSuffix) => {
    entries.forEach(e => {
      const key = teacher+":"+day+"-"+e.col, assigned = previewAssignments[key], isAssigned = !!assigned;
      if (isAssigned) successCount++; count++;
      html += `<tr class="${isAssigned?"":"table-danger"}">
        <td>${teacher}${badgeSuffix}</td>
        <td style="text-align:center">${e.col+1}</td>
        <td>${e.subject||""}</td>
        <td>${assigned ? `<strong>${assigned}</strong>` : `<span class="badge bg-danger">NOT ASSIGNED</span>`}</td>
      </tr>`;
    });
  };
  ...
  absentTeachers.forEach(teacher => {
    const data = loadTeacher(teacher); if (!data) return;
    appendRows(teacher, data.entries.filter(e=>e.row==day&&e.type==="lesson").sort((a,b)=>a.col-b.col), "");
  });
  Object.entries(partialAbsentTeachers).forEach(([teacher, absentPeriods]) => {
    const data = loadTeacher(teacher); if (!data) return;
    appendRows(teacher, data.entries.filter(e=>e.row==day&&e.type==="lesson"&&absentPeriods.includes(e.col)).sort((a,b)=>a.col-b.col), ` <span class="badge bg-info">Partial</span>`);
  });
  ...
  if (strategy==="roundRobin") html += `<small class="text-muted d-block mt-2"><i>Round-Robin cycles through available teachers sequentially.</i></small>`;
  else html += `<small class="text-muted d-block mt-2"><i>Fair strategy assigns to teachers with fewest total covers.</i></small>`;

  document.getElementById("previewAssignmentsList").innerHTML = html;

  let warningsHtml = "";
  if (result.conflicts > 0)   warningsHtml += `<strong>⚠️ ${result.conflicts} lessons cannot be assigned</strong><br><small>No suitable teachers available with current fairness constraints.</small><br>`;
  if (conflicts.length > 0)   warningsHtml += `<strong>⚠️ Back-to-back conflicts detected (${conflicts.length})</strong><br><small>Same teacher assigned to consecutive periods.</small><br>`;
...
function applyPreviewAssignments() {
  ...
  getCoverNeededLessons(day).forEach(item => {
    ...
    addCoverHistoryEntry(item.coveredTeacher, assignedCover, item.periodCol+1, day+1, item.entry?.subject||item.entry?.type||"Unknown", item.entry?.className||"Unknown", item.entry?.venue||"Unknown");
  });
  ...
  renderGrid();
  alert(`Auto-assign complete! ${Object.keys(previewAssignments).length} assignments applied.`);
}

// ── Generate Auto-Assignments (Fair) ──────────────────────────
function generateAutoAssignments() {
  const day = parseInt(document.getElementById("absenceDaySelect").value);
  const assignments = {}, metrics = loadMetrics();
  let assignmentsMade = 0, conflicts = 0;
  ...
  const processTeacher = (teacher, lessons, filterFn) => {
    lessons.forEach(e => {
      ...
      const key = teacher+":"+day+"-"+e.col;
      if (coverAssignments[key] || noCoverNeeded[key]) return;
      let avail = getAvailableTeachers(e.col, day, absentTeachers).filter(t => !Object.values(assignments).includes(t.name));
      if (filterFn) avail = avail.filter(filterFn);
      if (avail.length > 0) {
        avail.sort((a,b) => (metrics[a.name]?.totalCovers||0) - (metrics[b.name]?.totalCovers||0));
        assignments[key] = avail[0].name; assignmentsMade++;
      } else { conflicts++; }
    });
  };
  ...
  absentTeachers.forEach(teacher => {
    const data = loadTeacher(teacher); if (!data) return;
    processTeacher(teacher, data.entries.filter(e=>e.row==day&&e.type==="lesson").sort((a,b)=>a.col-b.col), null);
  });
  Object.entries(partialAbsentTeachers).forEach(([teacher, absentPeriods]) => {
    const data = loadTeacher(teacher); if (!data) return;
    processTeacher(teacher, data.entries.filter(e=>e.row==day&&e.type==="lesson"&&absentPeriods.includes(e.col)).sort((a,b)=>a.col-b.col), t => t.name !== teacher);
  });
  return { assignments, noCoverAssignments: {}, assignmentsMade, conflicts };
}
...
function autoAssignDayBalancing() {
  const day = parseInt(document.getElementById("absenceDaySelect").value);
  const allAvail = getAllAvailableTeachers(day, absentTeachers);
  const assignments = {}, coversAssignedToday = {};
  let assignmentsMade = 0, conflicts = 0;
  ...
  const processTeacher = (teacher, lessons, extraFilter) => {
    lessons.forEach(e => {
      ...
      if (avail.length > 0) {
        avail.sort((a,b) => { const aT=coversAssignedToday[a.name]||0, bT=coversAssignedToday[b.name]||0; return aT!==bT ? aT-bT : a.totalCovers-b.totalCovers; });
        const best = avail[0];
        assignments[key] = best.name;
        coversAssignedToday[best.name] = (coversAssignedToday[best.name]||0) + 1;
        assignmentsMade++;
      } else { conflicts++; }
    });
  };
...
function detectBackToBackConflicts(assignments) {
  ...
  Object.entries(assignments).forEach(([key, teacher]) => {
    const [absent, dayCol] = key.split(":"), [dayNum, period] = dayCol.split("-"), periodNum = parseInt(period);
    const nextKey = absent+":"+dayNum+"-"+(periodNum+1);
    if (assignments[nextKey] === teacher) conflicts.push({ teacher, absent, periods: `${periodNum+1}-${periodNum+2}`, type:"back-to-back" });
  });
  return conflicts;
}
...
function calculateFairnessScore(history) {
  if (history.length === 0) return { score:100, details:"No data", min:0, max:0, avg:"0", ratio:"0" };
  const totals = {};
  for (const h of history) totals[h.coverTeacher] = (totals[h.coverTeacher]||0)+1;
  const values = Object.values(totals);
  if (values.length <= 1) return { score:100, details:"Only one teacher", min:values[0]||0, max:values[0]||0, avg:(values[0]||0).toFixed(1), ratio:"1.00" };
  const min = Math.min(...values), max = Math.max(...values), avg = values.reduce((s,v)=>s+v,0)/values.length;
  const ratio = (max-min)/avg, score = Math.max(0, Math.min(100, 100-ratio*10));
  return { score:Math.round(score), min, max, avg:avg.toFixed(1), ratio:ratio.toFixed(2) };
}

document.getElementById("undoAutoAssignBtn").onclick = () => {
  if (!preAutoAssignState) { alert("Nothing to undo."); return; }
  if (confirm("Undo auto-assign? This will revert to the state before auto-assign was applied.")) {
    coverAssignments   = preAutoAssignState.coverAssignments;
    noCoverNeeded      = preAutoAssignState.noCoverNeeded;
    preAutoAssignState = null; previewAssignments = {}; previewNoCoverNeeded = {};
    addToHistoryLog("AUTO_ASSIGN_UNDONE", {});
    renderGrid(); alert("Auto-assign reverted.");
  }
};

document.getElementById("confirmAutoAssignBtn").onclick = () => {
  bootstrap.Modal.getInstance(document.getElementById("previewAutoAssignModal"))?.hide();
...
document.getElementById("bulkInput").addEventListener("change", async e => {
  ...
  for (const f of files) {
    ...
    teacherCache[name] = data; _buildEntryIndex(name, data); _teacherNamesCache = null; count++;
  }
  document.getElementById("status").innerText = "Imported " + count;
  refreshTeachers(); scheduleRenderGrid();
});

// ── Clear / Reset ──────────────────────────────────────────────
document.getElementById("clearBtn").onclick = () => {
  if (confirm("Clear absent teachers and today's cover assignments?\n\nThis will NOT erase cover history (needed for fairness tracking).")) {
    absentTeachers = []; coverAssignments = {}; noCoverNeeded = {}; absentTeacherReasons = {};
    addToHistoryLog("CLEAR_DAY_DATA", {});
    document.getElementById("status").innerText = "Absent teachers and assignments cleared. History preserved.";
    renderAbsentTeachersTable(); renderGrid();
  }
};

document.getElementById("clearAllBtn").onclick = () => {
  const confirmed = confirm("⚠️ FULL SYSTEM RESET - This cannot be undone!\n\nThis will clear:\n• All teacher timetables\n• Cover history\n• Metrics and fairness data\n\nAn automated backup will be exported.\n\nType 'CLEAR ALL' in the next prompt to confirm.");
  if (!confirmed) return;
  const userInput = prompt("Type 'CLEAR ALL' to confirm full system reset:");
  if (userInput !== "CLEAR ALL") { alert("Reset cancelled."); return; }
  backupCoverHistory(); autoExportAllData(); addToHistoryLog("FULL_SYSTEM_RESET", { action:"All data cleared with backups exported" });
  const keysToRemove = [];
  for (let i=0,len=localStorage.length; i<len; i++) {
    const k = localStorage.key(i);
    if (k.startsWith(PREFIX)||k==="coverPlans"||k===METRICS_KEY||k===HISTORY_KEY) keysToRemove.push(k);
  }
  keysToRemove.forEach(k => localStorage.removeItem(k));
  teacherCache={}; coverAssignments={}; noCoverNeeded={}; tallies={}; absentTeacherReasons={};
  _metricsCache=null; _historyCache=null; _teacherNamesCache=null; _startDateObj=null;
  absentTeachers=[]; for (const k in _entryIndex) delete _entryIndex[k];
  document.getElementById("status").innerText = "Full system reset complete. Backup exported.";
  refreshTeachers(); renderAbsentTeachersTable(); renderGrid();
};

document.getElementById("exportBtn").onclick = () => {
  backupCoverHistory();
  const data = { coverAssignments, noCoverNeeded, metrics:loadMetrics(), history:loadCoverHistory(), tenWeekStart:getCachedTenWeekStart() };
  const blob = new Blob([JSON.stringify(data,null,2)],{type:"application/json"});
  const a = document.createElement("a"); a.href=URL.createObjectURL(blob); a.download=`cover_backup_${new Date().toISOString().split("T")[0]}.json`; a.click();
...
document.getElementById("importMetricsInput").addEventListener("change", async e => {
  ...
  coverAssignments = data.coverAssignments || {}; noCoverNeeded = data.noCoverNeeded || {};
  saveMetrics(data.metrics||{}); if (data.history) saveCoverHistory(data.history);
  if (data.tenWeekStart) setCachedTenWeekStart(data.tenWeekStart);
  renderGrid(); alert("Backup restored successfully!");
});

// ── Excel export ───────────────────────────────────────────────
document.getElementById("exportExcelBtn").onclick = () => {
  if (!window.XLSX) { alert("Excel library not loaded."); return; }
  const history = loadCoverHistory(), metrics = loadMetrics();
  if (history.length===0 && Object.keys(metrics).length===0) { alert("No cover data to export."); return; }
  const wb = XLSX.utils.book_new(), currentWeek = getWeekNumber(coverDate), histStats = buildHistoryStats(history, currentWeek);
  ...
  for (let i=0;i<history.length;i++) {
    const h=history[i]; if (!weekCounts[h.coverTeacher]) weekCounts[h.coverTeacher]={};
  ...
  teachers.forEach(t => { const row=[t]; for(let w=1;w<=10;w++) row.push((weekCounts[t]&&weekCounts[t][w])||0); weeklyData.push(row); });
  ...
  const totalCovers=history.length, uniqueTeachers=teachers.size;
  const coverCounts=Array.from(teachers).map(t=>histStats[t]?.total||0);
  const minC=Math.min(...coverCounts), maxC=Math.max(...coverCounts);
  const statsData=[["Statistic","Value"],["Total Cover Sessions",totalCovers],["Number of Teachers",uniqueTeachers],["Average Covers per Teacher",(totalCovers/uniqueTeachers).toFixed(2)],["Minimum Covers",minC],["Maximum Covers",maxC],["Fairness Ratio (Max/Min)",minC>0?(maxC/minC).toFixed(2):"N/A"]];
...
function backupCoverHistory() {
  const history = loadCoverHistory();
  const backup  = { date:new Date().toISOString(), historyCount:history.length, data:history };
  const backups = JSON.parse(localStorage.getItem(HISTORY_BACKUP_KEY)||"[]");
  backups.push(backup); if (backups.length>10) backups.shift();
...
function addToHistoryLog(action, details) {
  ...
  log.push({ timestamp:new Date().toISOString(), action, details, dataSnapshot:{ historyCount:loadCoverHistory().length, metricsCount:Object.keys(loadMetrics()).length } });
  const threeMonthsAgo = new Date(); threeMonthsAgo.setMonth(threeMonthsAgo.getMonth()-3);
  const filtered = log.filter(e => new Date(e.timestamp) > threeMonthsAgo);
  if (filtered.length>100) { const old=log.slice(0,log.length-100); if(old.length>0) autoExportHistoryLog(old); localStorage.setItem(HISTORY_LOG_KEY, JSON.stringify(filtered.slice(-100))); }
  else localStorage.setItem(HISTORY_LOG_KEY, JSON.stringify(filtered));
}

function autoExportHistoryLog(entries) {
  const data = { exportDate:new Date().toISOString(), archiveType:"history-log-archive", entries };
  const blob = new Blob([JSON.stringify(data,null,2)],{type:"application/json"});
  const a = document.createElement("a"); a.href=URL.createObjectURL(blob); a.download=`cover_history_log_archive_${new Date().toISOString().split("T")[0]}.json`; a.click();
  setTimeout(()=>URL.revokeObjectURL(a.href),1000);
}

function autoExportAllData() {
  const data = { exportDate:new Date().toISOString(), exportType:"full-system-backup", coverHistory:loadCoverHistory(), metrics:loadMetrics(), tenWeekStart:getCachedTenWeekStart(), historyLog:JSON.parse(localStorage.getItem(HISTORY_LOG_KEY)||"[]") };
  const blob = new Blob([JSON.stringify(data,null,2)],{type:"application/json"});
  const a = document.createElement("a"); a.href=URL.createObjectURL(blob); a.download=`cover_system_backup_${new Date().toISOString().split("T")[0]}.json`; a.click();
  setTimeout(()=>URL.revokeObjectURL(a.href),1000);
  return true;
}
...
document.getElementById("teacherSearchInput").addEventListener("input", e => {
  ...
  const allNames = getTeacherNames().filter(n => !new Set(absentTeachers).has(n));
  const matches  = allNames.filter(name => name.toLowerCase().includes(query));
  if (matches.length === 0) {
    searchResults.innerHTML = '<div class="list-group-item text-muted">No teachers found</div>';
...
document.addEventListener("click", e => {
  if (!e.target.closest("#teacherSearchInput") && !e.target.closest("#searchResults"))
...
document.addEventListener("keydown", e => {
  if ((e.ctrlKey||e.metaKey) && e.key>="1" && e.key<="4") { e.preventDefault(); focusStep(parseInt(e.key)); }
  if ((e.ctrlKey||e.metaKey) && e.key==="ArrowRight") { e.preventDefault(); focusStep(Math.min(4,currentWorkflowStep+1)); }
  if ((e.ctrlKey||e.metaKey) && e.key==="ArrowLeft")  { e.preventDefault(); focusStep(Math.max(1,currentWorkflowStep-1)); }
  if ((e.altKey||e.metaKey) && e.key.toLowerCase()==="a" && currentWorkflowStep>=2) { e.preventDefault(); if(hasAnyAbsences()) document.getElementById("autoAssignBtn")?.click(); }
  if ((e.ctrlKey||e.metaKey) && e.shiftKey && e.key.toLowerCase()==="a" && currentWorkflowStep>=2) {
    e.preventDefault();
    if (!hasAnyAbsences()) { alert("No absent teachers selected."); return; }
    const strategy = document.getElementById("autoAssignStrategy").value;
    let result;
    ...
    previewAssignments=result.assignments; previewNoCoverNeeded=result.noCoverAssignments;
    applyPreviewAssignments(); alert(`✓ Auto-assigned! ${result.assignmentsMade} covers assigned.`);
  }
  if ((e.altKey||e.metaKey) && e.key.toLowerCase()==="e" && currentWorkflowStep>=3) { e.preventDefault(); document.getElementById("exportExcelBtn")?.click(); }
});

// ── Absence Reasons: Settings modal ───────────────────────────
function populateAbsenceReasonsList() {
  const reasons = getAbsenceReasons(), list = document.getElementById("absenceReasonsList");
  if (!list) return;
...
document.getElementById("addAbsenceReasonBtn")?.addEventListener("click", () => {
  const input = document.getElementById("newAbsenceReasonInput");
  if (input && input.value.trim()) { if (addAbsenceReason(input.value)) { input.value=""; populateAbsenceReasonsList(); } }
});
document.getElementById("resetAbsenceReasonsBtn")?.addEventListener("click", () => {
  if (confirm("Reset absence reasons to defaults?")) { saveAbsenceReasons(DEFAULT_ABSENCE_REASONS); populateAbsenceReasonsList(); }
});
...
document.getElementById("autoAssignStrategy").addEventListener("change", e => saveLastStrategy(e.target.value));
...
document.addEventListener("show.bs.modal", e => {
  if      (e.target.id==="historyModal")          displayCoverHistory();
    ...
    document.getElementById("freePeriodsOnlySwitch").checked  = s.freePeriodsOnly;
    ...
    document.getElementById("freePeriodsOnlySwitch2").checked = s.freePeriodsOnly;
    ...
    if (tenWeekStart) document.getElementById("tenWeekStartDate2").value = tenWeekStart;
  }
});

document.getElementById("saveFairnessSettings2")?.addEventListener("click", () => {
  const s = { excludeDnd:document.getElementById("excludeDndSwitch2").checked, freePeriodsOnly:document.getElementById("freePeriodsOnlySwitch2").checked, maxCoversPerDay:parseInt(document.getElementById("maxCoversPerDay2").value), maxCoversPerWeek:parseInt(document.getElementById("maxCoversPerWeek2").value), useLastResort:document.getElementById("useLastResortSwitch2").checked };
  saveFairnessSettings(s); alert("✓ Fairness settings saved!");
});

function populateAbsenceReasonsList2() {
  const reasons = getAbsenceReasons(), list = document.getElementById("absenceReasonsList2");
  list.innerHTML = reasons.map(r => `
    <div class="list-group-item d-flex justify-content-between align-items-center">
      ${r}<button class="btn btn-sm btn-danger" onclick="removeAbsenceReason2('${r}')">Remove</button>
    </div>`).join("");
}
function removeAbsenceReason2(reason) { removeAbsenceReason(reason); populateAbsenceReasonsList2(); }
document.getElementById("addAbsenceReasonBtn2")?.addEventListener("click", () => {
  const input = document.getElementById("newAbsenceReason2"), reason = input.value.trim();
  if (reason && addAbsenceReason(reason)) { input.value=""; populateAbsenceReasonsList2(); }
});
document.getElementById("resetAbsenceReasonsBtn2")?.addEventListener("click", () => {
  if (confirm("Reset to default absence reasons?")) { saveAbsenceReasons(DEFAULT_ABSENCE_REASONS); populateAbsenceReasonsList2(); alert("✓ Absence reasons reset to defaults!"); }
});
...
document.getElementById("saveTenWeekPeriod2")?.addEventListener("click", () => {
  ...
  if (dateStr) { localStorage.setItem(TEN_WEEK_START, dateStr); alert("✓ 10-week period start saved!"); }
});
document.getElementById("resetTenWeekPeriod2")?.addEventListener("click", () => {
  if (confirm("⚠️ This will CLEAR ALL history and fairness data. Continue?")) {
    localStorage.removeItem(HISTORY_KEY); localStorage.removeItem(METRICS_KEY); localStorage.removeItem(TEN_WEEK_START);
    alert("✓ Period reset complete. History cleared."); location.reload();
  }
});
...
document.getElementById("workflowBody")?.addEventListener("shown.bs.collapse",  () => { document.getElementById("workflowToggleIcon").textContent="▼"; });
document.getElementById("workflowBody")?.addEventListener("hidden.bs.collapse", () => { document.getElementById("workflowToggleIcon").textContent="▶"; });

document.getElementById("savePeriodBtn").onclick = () => {
  const newStartDate = document.getElementById("tenWeekStartDate").value;
  if (newStartDate) { setCachedTenWeekStart(newStartDate); updatePeriodStatus(); updateWeekDisplay(); scheduleRenderGrid(); alert("10-week period updated!"); }
};
document.getElementById("resetPeriodBtn").onclick = () => {
  if (confirm("Reset the 10-week period? This will mark today as Week 1.")) {
    const today = new Date().toISOString().split("T")[0];
    setCachedTenWeekStart(today); document.getElementById("tenWeekStartDate").value=today;
    updatePeriodStatus(); coverDate=today; document.getElementById("coverDate").value=today;
    updateWeekDisplay(); scheduleRenderGrid(); alert("10-week period has been reset!");
  }
};
...
function focusStep(step) {
  ...
  let prerequisitesMet = true, warnings = [];
  if (stepNum>=2 && !document.getElementById("absenceDaySelect").value) { warnings.push("⚠️ Step 1: Please select a day first"); prerequisitesMet=false; }
  if (stepNum>=3 && absentTeachers.length===0) { warnings.push("⚠️ Step 2: Please add absent teachers first"); prerequisitesMet=false; }
  if (stepNum>=4 && Object.keys(coverAssignments).length===0) { warnings.push("⚠️ Step 3: Please run auto-assign or assign covers manually first"); prerequisitesMet=false; }
  if (!prerequisitesMet && stepNum>currentWorkflowStep) alert(warnings.join("\n")+"\n\nYou can still continue, but complete these steps for best results.");
  currentWorkflowStep=stepNum; updateWorkflowUI(stepNum); scrollToStep(stepNum);
}
function updateWorkflowUI(step) {
  ...
  for (let i=1;i<=4;i++) {
    ...
    if (i<step)  { ind.textContent="✓"; ind.className="step-indicator text-success fw-bold"; }
    else if (i===step) { ind.textContent="●"; ind.className="step-indicator text-primary fw-bold"; }
    else         { ind.textContent="○"; ind.className="step-indicator text-muted"; }
  }
}
function scrollToStep(step) {
  const targets = [null,"absenceDaySelect","teacherSearchInput","autoAssignStrategy","exportExcelBtn"];
  const el = targets[step] ? document.getElementById(targets[step]) : null;
  if (el) setTimeout(()=>{ el.scrollIntoView({behavior:"smooth",block:"nearest"}); el.focus(); },100);
}
document.getElementById("absenceDaySelect")?.addEventListener("change", () => { if(currentWorkflowStep===1){ currentWorkflowStep=2; updateWorkflowUI(2); } });
document.getElementById("addAbsenceTeacherBtn")?.addEventListener("click", () => { if(absentTeachers.length>0&&currentWorkflowStep===2){ currentWorkflowStep=3; updateWorkflowUI(3); } });

// ── Fairness Settings Modal ────────────────────────────────────
document.getElementById("saveFairnessBtn").onclick = () => {
  const s = { excludeDnd:document.getElementById("excludeDndSwitch").checked, freePeriodsOnly:document.getElementById("freePeriodsOnlySwitch").checked, maxCoversPerDay:parseInt(document.getElementById("maxCoversPerDay").value), maxCoversPerWeek:parseInt(document.getElementById("maxCoversPerWeek").value), useLastResort:document.getElementById("useLastResortSwitch").checked };
  saveFairnessSettings(s); addToHistoryLog("FAIRNESS_SETTINGS_UPDATED",s); alert("Fairness settings saved successfully!");
  bootstrap.Modal.getInstance(document.getElementById("fairnessSettingsModal"))?.hide();
  scheduleRenderGrid();
};
document.getElementById("resetFairnessBtn").onclick = () => {
  if (confirm("Reset fairness settings to defaults?")) {
    ...
    document.getElementById("excludeDndSwitch").checked    = DEFAULT_FAIRNESS_SETTINGS.excludeDnd;
    document.getElementById("maxCoversPerDay").value       = DEFAULT_FAIRNESS_SETTINGS.maxCoversPerDay;
    document.getElementById("maxCoversPerWeek").value      = DEFAULT_FAIRNESS_SETTINGS.maxCoversPerWeek;
    document.getElementById("useLastResortSwitch").checked = DEFAULT_FAIRNESS_SETTINGS.useLastResort;
    addToHistoryLog("FAIRNESS_SETTINGS_RESET",DEFAULT_FAIRNESS_SETTINGS); alert("Fairness settings reset to defaults!"); scheduleRenderGrid();
  }
};
...
async function generatePDF() {
  try {
    ...
    if (absentTeachers.length===0) { alert("No absent teachers selected."); return; }
    ...
    let y = 15;
    doc.setFontSize(16); doc.text(`Absent Teachers Cover Plan - Day ${day}`,14,y); y+=10; doc.setFontSize(10);
    absentTeachers.forEach(teacher => {
      const data = loadTeacher(teacher); if (!data) return;
      const lessons = data.entries.filter(e=>e.row==day-1&&e.type==="lesson").sort((a,b)=>a.col-b.col);
      if (lessons.length===0) return;
      doc.setFont(undefined,"bold"); doc.text(teacher,14,y); y+=6; doc.setFont(undefined,"normal");
      lessons.forEach(e => {
        const key = teacher+":"+(day-1)+"-"+e.col, assigned = coverAssignments[key]||"⚠ NOT ASSIGNED";
        const line = `P${e.col+1} | ${e.subject||""} | ${e.className||""} | ${e.venue||""} | Cover: ${assigned}`;
        const split = doc.splitTextToSize(line,180); doc.text(split,16,y); y+=split.length*5;
...
function setWorkflowBtnExpanded(expanded) {
  const btn = document.getElementById("workflowToggleBtn"); if (!btn) return;
  btn.setAttribute("aria-expanded", expanded?"true":"false");
}
document.addEventListener("DOMContentLoaded", () => {
  const wrapper = document.getElementById("workflowSidebarCollapse"), btn = document.getElementById("workflowToggleBtn");
  if (!wrapper||!btn||!window.bootstrap) return;
  const saved = localStorage.getItem(WORKFLOW_SIDEBAR_STATE_KEY), visible = saved===null ? true : saved==="true";
  const collapse = bootstrap.Collapse.getOrCreateInstance(wrapper,{toggle:false});
  if (visible) collapse.show(); else collapse.hide();
  setWorkflowBtnExpanded(visible);
  wrapper.addEventListener("shown.bs.collapse",  ()=>{ localStorage.setItem(WORKFLOW_SIDEBAR_STATE_KEY,"true");  setWorkflowBtnExpanded(true);  });
  wrapper.addEventListener("hidden.bs.collapse", ()=>{ localStorage.setItem(WORKFLOW_SIDEBAR_STATE_KEY,"false"); setWorkflowBtnExpanded(false); });
});
...
async function cloudSaveTo() {
  ...
  try {
    ...
    addToHistoryLog("CLOUD_SAVE", { date: coverDate, schoolId: FirebaseAdapter.getStoredSchoolId() });
    alert("✅ Data saved to cloud successfully.");
  } catch (e) {
    alert("❌ Cloud save failed:\n" + e.message);
    console.error("cloudSaveTo:", e);
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = "💾 Save To"; }
  }
}

// ── Firebase: Cloud Read ───────────────────────────────────────
async function cloudReadFrom() {
  if (!confirm("Read from cloud?\n\nCloud data will be merged with your current local data.")) return;
  const btn = document.getElementById("cloudReadBtn");
  try {
    if (btn) { btn.disabled = true; btn.textContent = "⏳ Reading…"; }
    const data = await FirebaseAdapter.readFrom(coverDate);

    // Merge daily allocation
    if (data.allocation) {
      const a = data.allocation;
      if (a.coverAssignments)      Object.assign(coverAssignments,      a.coverAssignments);
      if (a.noCoverNeeded)         Object.assign(noCoverNeeded,         a.noCoverNeeded);
      if (a.absentTeachers)        absentTeachers        = [...new Set([...absentTeachers, ...a.absentTeachers])];
      if (a.partialAbsentTeachers) Object.assign(partialAbsentTeachers, a.partialAbsentTeachers);
      if (a.absentTeacherReasons)  Object.assign(absentTeacherReasons,  a.absentTeacherReasons);
    }

    // Merge history (de-duplicate by composite key)
    if (data.history?.length) {
      const existing    = loadCoverHistory();
      const existingKeys = new Set(
        existing.map(h => `${h.date}_${h.coveredTeacher}_P${h.period}_${h.coverTeacher}`)
      );
    ...
    // Merge metrics (cloud fills gaps, local takes priority)
    if (data.metrics && Object.keys(data.metrics).length) {
      const m = loadMetrics();
      Object.assign(data.metrics, m); // local wins on conflict
      saveMetrics(data.metrics);
    }

    // Apply settings
    if (data.settings) {
      if (data.settings.fairnessSettings) saveFairnessSettings(data.settings.fairnessSettings);
      if (data.settings.tenWeekStart)     setCachedTenWeekStart(data.settings.tenWeekStart);
      if (data.settings.absenceReasons)   saveAbsenceReasons(data.settings.absenceReasons);
    }

    addToHistoryLog("CLOUD_READ", { date: coverDate, schoolId: FirebaseAdapter.getStoredSchoolId() });
    refreshTeachers();
    renderAbsentTeachersTable();
    scheduleRenderGrid();
    ...
    if (btn) { btn.disabled = false; btn.textContent = "📥 Read From"; }
  }
}
...
function initCloudUI() {
  ...
  if (FirebaseAdapter.isConfigured()) {
    ...
    if (dot)   { dot.textContent = "🟢"; dot.title = "Firebase connected"; }
    if (label) label.textContent = `School: ${schoolId}`;
    if (saveBtn) saveBtn.disabled = false;
    if (readBtn) readBtn.disabled = false;
    FirebaseAdapter.init();
  } else {
    if (dot)   { dot.textContent = "🔴"; dot.title = "Firebase not configured"; }
...
document.getElementById("cloudConfigureBtn")?.addEventListener("click", () => {
  ...
  document.getElementById("settingsModal").addEventListener("shown.bs.modal", () => {
    const firebaseTab = document.querySelector('[data-bs-target="#firebaseTab"]');
...
function saveFirebaseSettings() {
  ...
  localStorage.setItem("firebaseConfig",   JSON.stringify({ apiKey, authDomain, projectId, storageBucket, messagingSenderId, appId }));
  localStorage.setItem("firebaseSchoolId", schoolId);

  initCloudUI();
  alert("✅ Firebase settings saved. Save To and Read From are now active in the ☁️ Cloud menu.");
}
...
document.getElementById("cloudReadBtn")?.addEventListener("click", cloudReadFrom);
document.getElementById("saveFirebaseSettingsBtn")?.addEventListener("click", saveFirebaseSettings);
