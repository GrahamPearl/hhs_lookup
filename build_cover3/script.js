  ...
  for (let period = 0; period < 6; period++) {
    ...
    let tdContent = avail.length === 0
      ...
      : avail.map(teacher => {
          ...
          const borderClass = overloaded ? " border border-danger" : "";
          const titleAttr   = lastFree ? ' title="⚠️ Last available period today"' : "";
          return `<span class="badge me-1 avail-badge ${badgeColor}${borderClass}" draggable="true" data-teacher-name="${teacher.name}"${titleAttr}>
  ${teacher.name}${lastFree ? " ⚠️" : ""}
...
function applyPreviewAssignments() {
  ...
  addToHistoryLog("AUTO_ASSIGN_APPLIED", { strategy: document.getElementById("autoAssignStrategy").value, assignmentsMade: Object.keys(previewAssignments).length });
  renderGrid();
