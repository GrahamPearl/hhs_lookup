...
/* ═══════════════════════════════════════════════════════════════
   REAL-TIME SYNC  — localStorage  "storage" event
   Fires in every tab that did NOT make the write.
   When the admin tab saves jobs, teacher tab auto-refreshes.
   ═══════════════════════════════════════════════════════════════ */
...
(function () {
  ...
  function showSyncToast(msg, type) {
    ...
    const inner = document.getElementById("syncToastInner");
    const icon  = document.getElementById("syncToastIcon");
    const text  = document.getElementById("syncToastMsg");
    if (!toast || !inner) return;

    const cfg = {
      info:    { cls: "alert-info",    ico: "🔄" },
      success: { cls: "alert-success", ico: "✅" },
      warning: { cls: "alert-warning", ico: "⚠️" },
    };
    const { cls, ico } = cfg[type] || cfg.info;

    // Reset classes
    inner.className = `alert mb-0 py-2 px-3 shadow ${cls}`;
  ...
  function setSyncPill(state) {
    ...
    const cfg = {
      ...
      offline: { cls: "badge-secondary", label: "○ Offline" },
    };
    const { cls, label } = cfg[state] || cfg.live;
    pill.className = `badge ${cls} ml-2`;
    pill.style.fontSize = ".7rem";
    pill.textContent = label;
  }

  /* ── Diff helper: detect what changed between old and new job lists ── */
  function diffJobs(oldMap, newList) {
    const changes = [];
    const newMap = new Map(newList.map((j) => [j.id, j]));

    // Changed or new
    newMap.forEach((newJob, id) => {
      const old = oldMap.get(id);
      if (!old) {
        changes.push({ type: "added", job: newJob });
      } else if (old.status !== newJob.status) {
        changes.push({ type: "status", job: newJob, from: old.status });
      }
    });

    // Deleted
    oldMap.forEach((_, id) => {
      if (!newMap.has(id)) changes.push({ type: "removed", id });
    });

    return changes;
  }

  /* ── Build human-readable summary for teacher ── */
  function summariseChanges(changes, teacherName) {
    // Only surface changes relevant to this teacher (if name is set)
    const q = teacherName ? teacherName.trim().toLowerCase() : "";

    const relevant = q
      ? changes.filter(
          (c) => c.job && (c.job.teacher || "").toLowerCase() === q
        )
      : changes;

    if (!relevant.length) return null; // Nothing for this teacher

    // Priority: status changes on teacher's own jobs
    const statusChange = relevant.find((c) => c.type === "status");
    if (statusChange) {
      const { job, from } = statusChange;
      const ref = job.reference || `#${job.id}`;
      return {
        msg: `${ref}: ${from} → ${job.status}`,
        type: job.status === "Completed" ? "success" : "info",
      };
    }

    const added = relevant.filter((c) => c.type === "added").length;
    const removed = relevant.filter((c) => c.type === "removed").length;
    if (added)   return { msg: `${added} new job${added > 1 ? "s" : ""} added`, type: "info" };
    if (removed) return { msg: `${removed} job${removed > 1 ? "s" : ""} removed`, type: "warning" };

    return { msg: "Jobs updated", type: "info" };
  }

  /* ── Core handler ── */
  function onStorageChange(e) {
    // Only react to jobs key
    if (e.key !== STORAGE_KEYS.JOBS) return;

    // Ignore null (key deleted / cleared)
    if (!e.newValue) return;

    setSyncPill("syncing");

    // Snapshot current in-memory jobs before reloading
    const oldMap = new Map(getAllJobs().map((j) => [j.id, j]));

    // Parse incoming data
    let newJobs;
    try {
      newJobs = safeJsonParse(e.newValue, []).map(normalizeJob);
    } catch {
      setSyncPill("live");
      return;
    }

    // Rebuild in-memory jobs map (no full AppState.load to avoid
    // clobbering settings that may also be mid-write in the other tab)
    jobs.clear();
    newJobs.forEach((j) => jobs.set(j.id, j));

    // Update idCounter if needed
    const maxId = Math.max(0, ...newJobs.map((j) => j.id));
    if (maxId > idCounter) idCounter = maxId;

    // If teacher view is active → re-render + toast
    if (teacherViewMode) {
      const changes = diffJobs(oldMap, newJobs);

      if (changes.length) {
        renderTeacherView();
        updateMatchBadge(teacherNameFilter);

        const summary = summariseChanges(changes, teacherNameFilter);
        if (summary) {
          showSyncToast(summary.msg, summary.type);
        }
      }
    } else {
      // Admin view: silent re-render (preserves existing behaviour)
      rerenderAll();
    }
  ...
  /* ── Online / offline pill ── */
  window.addEventListener("online",  () => setSyncPill("live"));
  window.addEventListener("offline", () => setSyncPill("offline"));

  /* ── Register storage listener ── */
  window.addEventListener("storage", onStorageChange);

  /* ── Patch AppState.save so the admin tab broadcasts immediately ──
     We wrap the existing save() so cross-tab sync works without any
     changes to the rest of the codebase.                             */
  const _originalSave = AppState.save.bind(AppState);
  AppState.save = function () {
    _originalSave();
    // The "storage" event only fires in OTHER tabs automatically.
    // Nothing extra needed — the native event handles it.
  };

  console.log("[RealTimeSync] localStorage storage listener registered");
})();
