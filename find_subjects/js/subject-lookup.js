...
(function(){
  const SUBJECT_FILES = { '10':'Subjects-Grade10.json', '11':'Subjects-Grade11.json', '12':'Subjects-Grade12.json' };
  ...
  function parseSubjectLineQuery(raw){
    let low = String(raw || '').trim().toLowerCase();
    if (!low) return null;
    let lineMatch = low.match(/(?:^|\s)line\s*[:=]?\s*(\d{1,2})/);
    let line = lineMatch ? lineMatch[1] : null;
    if (!line) { const tail = low.match(/(\d{1,2})\s*$/); if (tail) line = tail[1]; }
  ...
  function findRows(grade, subjQuery, line){
    ...
    const seen = new Set();
    return out.filter(r => { const id = String(r.student_number||'')+'|'+String(r.teacher||''); if (seen.has(id)) return false; seen.add(id); return true; });
  }
  ...
  function renderTable(subjectLabel, lineLabel, rows){
    ...
    if (!rows.length){
      $area.html(`<div class="alert alert-warning mt-3">No students found for <strong>${esc(subjectLabel)}</strong>${lineLabel? ' (Line '+esc(lineLabel)+')':''}.</div>`);
      return;
    }
  ...
  $(function(){
    ...
    $form.on('submit', async function(e){
      ...
      if (!grade){
        if (typeof showMessage==='function') showMessage('warning','Please select a <strong>Grade</strong> to search by Subject + Line.');
        else $('#messageArea').html('<div class="alert alert-warning">Please select a <strong>Grade</strong>.</div>');
        return;
      }
      try {
        ...
        const subjLabel = (parsed.subj||'Subject').replace(/\w/g,c=>c.toUpperCase());
        renderTable(subjLabel, parsed.line, list);
        if (typeof showMessage==='function') {
          if (!list.length) showMessage('warning', `No results for: <strong>${esc(raw)}</strong>.`);
          else showMessage('info', `${list.length} result${list.length===1?'':'s'} found.`);
        }
      } catch(err){
        const msg = 'Could not load subject data. ' + esc(err.message);
        if (typeof showMessage==='function') showMessage('danger', msg);
        else $('#messageArea').html('<div class="alert alert-danger">'+msg+'</div>');
      }
    });
  });
})();
