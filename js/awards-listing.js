...
const STORAGE_KEY = "activityAwards";
const DATA_URL = "../students.json";
const LIST_URL_CLASSES = "list_of_classes.txt";
const LIST_URL_ACTIVITY = "list_of_awards.txt";
...
async function loadAgeGroups() {
  const res = await fetch(LIST_URL_CLASSES);
  const text = await res.text();
  const lines = text.split("\n");
  lines.forEach((line) => {
    if (!line.trim()) return;
...
function populateDropdownAgeGroup() {
  ...
  select.innerHTML = '<option value="">-- Select Reg Class --</option>';
...
function exportTeam() {
  ...
  a.href = url;
  a.download = `${(act || "Activity").replace(/\s/g, "_")}_${(age || "Age").replace(/\s/g, "_")}_Class.json`;
  a.click();
}
...
function exportTeamExcel() {
  ...
  if (!act || !age) {
    alert("Please select an Award and Reg Class Group first.");
    return;
  }
  ...
  const filename = `${act.replace(/\s/g, "_")}_${age.replace(/\s/g, "_")}_class.xlsx`;
...
function importTeam() {
  ...
  reader.onload = function (e) {
    const key = currentKey();
    if (!key) {
      alert("Please select an Award and Reg Class before importing.");
      return;
    }
    try {
